warning("package ggheatmap should be installed as version 2.11, or else the theme function will be incompatible")
...
pltdata.phylo.heatmap = pltdata.phylo.heatmap %>% dplyr::select(-tipLabelMatched) %>%
  ...
  dplyr::rename(
    ...
    R_Rr25 = SRR25, R_SRA = SRA, R_RPC = RPC, R_RCC = RCC,
    L_LMA = LMA, L_LNC = LNC, L_LPC = LPC, L_LCC = LCC, L_Ld13C = Ld13C,
    L_Rd25 = Rdark25P, L_Vcmax25 = Vcmax25, L_Asat = Asat, L_LA = LA
  )
...
ggsave(filename = "outplts/SIFigures/FigS1-TraitHeatmap.pdf", plot = plt.phylo.total, width = 6.3, height = 6)
