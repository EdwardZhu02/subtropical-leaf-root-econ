...
# 
model.sma.SRR25_Rdark25P_total = sma(traitData_touse$SRR25 ~ traitData_touse$Rdark25P)
model.sma.SRR25_Rdark25P_tree = sma(traitData_touse_tree$SRR25 ~ traitData_touse_tree$Rdark25P)
model.sma.SRR25_Rdark25P_shrub = sma(traitData_touse_shrub$SRR25 ~ traitData_touse_shrub$Rdark25P)

#plot(model.sma.SRR25_Rdark25P)
summary(model.sma.SRR25_Rdark25P_total)
summary(model.sma.SRR25_Rdark25P_tree)
summary(model.sma.SRR25_Rdark25P_shrub)

smaplt_total_intercept = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_total_intercept_lower = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][1,2]
smaplt_total_intercept_upper = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][1,3] 
smaplt_total_slope = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_total_slope_lower = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][2,2]
smaplt_total_slope_upper = model.sma.SRR25_Rdark25P_total[["coef"]][[1]][2,3]

smaplt_tree_intercept = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_tree_intercept_lower = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][1,2]
smaplt_tree_intercept_upper = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][1,3] 
smaplt_tree_slope = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_tree_slope_lower = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][2,2]
smaplt_tree_slope_upper = model.sma.SRR25_Rdark25P_tree[["coef"]][[1]][2,3]

smaplt_shrub_intercept = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_shrub_intercept_lower = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][1,2]
smaplt_shrub_intercept_upper = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][1,3] 
smaplt_shrub_slope = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_shrub_slope_lower = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][2,2]
smaplt_shrub_slope_upper = model.sma.SRR25_Rdark25P_shrub[["coef"]][[1]][2,3]


smaplt_total_data = traitData_touse %>% dplyr::select(speciesFullName, GrowthForm, SiteID, SRR25, Rdark25P) %>%
  dplyr::mutate(Rdark25P = as.numeric(Rdark25P), SRR25 = as.numeric(SRR25))
smaplt_total_data$predicted_SRR25 = smaplt_total_intercept + smaplt_total_slope * smaplt_total_data$Rdark25P
smaplt_total_data$predicted_SRR25_upper_total = smaplt_total_intercept_upper + smaplt_total_slope_upper * smaplt_total_data$Rdark25P
smaplt_total_data$predicted_SRR25_lower_total = smaplt_total_intercept_lower + smaplt_total_slope_lower * smaplt_total_data$Rdark25P

smaplt_tree_data = traitData_touse_tree %>% dplyr::select(speciesFullName, GrowthForm, SiteID, SRR25, Rdark25P) %>%
  dplyr::mutate(Rdark25P = as.numeric(Rdark25P), SRR25 = as.numeric(SRR25))
smaplt_tree_data$predicted_SRR25 = smaplt_tree_intercept + smaplt_tree_slope * smaplt_tree_data$Rdark25P
smaplt_tree_data$predicted_SRR25_upper_tree = smaplt_tree_intercept_upper + smaplt_tree_slope_upper * smaplt_tree_data$Rdark25P
smaplt_tree_data$predicted_SRR25_lower_tree = smaplt_tree_intercept_lower + smaplt_tree_slope_lower * smaplt_tree_data$Rdark25P

smaplt_shrub_data = traitData_touse_shrub %>% dplyr::select(speciesFullName, GrowthForm, SiteID, SRR25, Rdark25P) %>%
  dplyr::mutate(Rdark25P = as.numeric(Rdark25P), SRR25 = as.numeric(SRR25))
smaplt_shrub_data$predicted_SRR25 = smaplt_shrub_intercept + smaplt_shrub_slope * smaplt_shrub_data$Rdark25P
smaplt_shrub_data$predicted_SRR25_upper_shrub = smaplt_shrub_intercept_upper + smaplt_shrub_slope_upper * smaplt_shrub_data$Rdark25P
smaplt_shrub_data$predicted_SRR25_lower_shrub = smaplt_shrub_intercept_lower + smaplt_shrub_slope_lower * smaplt_shrub_data$Rdark25P
...
model.sma.SRR25_Rdark25P_annotlabel = paste0(
  ...
  " P",
  ifelse(
    signif(as.numeric(model.sma.SRR25_Rdark25P_total[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.SRR25_Rdark25P_total[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.SRR25_Rdark25P_total[["pval"]]), digits = 2), "\n"))),
  
  "Tree:R=",
  round(ifelse(
    as.numeric(model.sma.SRR25_Rdark25P_tree[["groupsummary"]][["Slope"]]) > 0,
    sqrt(as.numeric(model.sma.SRR25_Rdark25P_tree[["r2"]])),
    -sqrt(as.numeric(model.sma.SRR25_Rdark25P_tree[["r2"]]))
  ), digits = 2), 
  " P", 
  ifelse(
    signif(as.numeric(model.sma.SRR25_Rdark25P_tree[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.SRR25_Rdark25P_tree[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.SRR25_Rdark25P_tree[["pval"]]), digits = 2), "\n"))),
  
  "Shrub:R=",
  round(ifelse(
    as.numeric(model.sma.SRR25_Rdark25P_shrub[["groupsummary"]][["Slope"]]) > 0,
    sqrt(as.numeric(model.sma.SRR25_Rdark25P_shrub[["r2"]])),
    -sqrt(as.numeric(model.sma.SRR25_Rdark25P_shrub[["r2"]]))
  ), digits = 2),
  " P", 
  ifelse(
    signif(as.numeric(model.sma.SRR25_Rdark25P_shrub[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.SRR25_Rdark25P_shrub[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.SRR25_Rdark25P_shrub[["pval"]]), digits = 2), "\n")))
)

smaplt.SRR25_Rdark25P = ggplot(smaplt_total_data, aes(x=Rdark25P, y=SRR25)) +
  geom_point(aes(color=GrowthForm), size=2, alpha=1.0) +  # data points
  stat_function( # total
    fun = function(x) smaplt_total_slope * x + smaplt_total_intercept, 
    xlim = c(min(smaplt_total_data$Rdark25P, na.rm=T), max(smaplt_total_data$Rdark25P, na.rm=T)),
    color = "black", linewidth = 1.0
  ) +
  stat_function( # tree
    fun = function(x) smaplt_tree_slope * x + smaplt_tree_intercept, 
    xlim = c(min(smaplt_tree_data$Rdark25P, na.rm=T), max(smaplt_tree_data$Rdark25P, na.rm=T)),
    color = "#F07673", linewidth = 1.0
  ) +
  stat_function( # shrub
    fun = function(x) smaplt_shrub_slope * x + smaplt_shrub_intercept, 
    xlim = c(min(smaplt_shrub_data$Rdark25P, na.rm=T), max(smaplt_shrub_data$Rdark25P, na.rm=T)),
    color = "#7998AD", linewidth = 1.0
  ) +
  labs(subtitle = "RES (Rr25-Rdark25P)", x = "lnRdark25P", y = "lnRr25") + 
  theme_minimal() + 
  theme(axis.line = element_line(colour = "black")) +
  theme(legend.position = "none") +
  scale_color_manual(values = c("#EECA40", "#7998AD", "#F07673")) +
...
# 
model.sma.LNC_RNC_total = sma(traitData_touse$LNC ~ traitData_touse$RNC)
model.sma.LNC_RNC_tree = sma(traitData_touse_tree$LNC ~ traitData_touse_tree$RNC)
model.sma.LNC_RNC_shrub = sma(traitData_touse_shrub$LNC ~ traitData_touse_shrub$RNC)

#plot(model.sma.LNC_RNC)
summary(model.sma.LNC_RNC_total)
summary(model.sma.LNC_RNC_tree)
summary(model.sma.LNC_RNC_shrub)

smaplt_total_intercept = model.sma.LNC_RNC_total[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_total_intercept_lower = model.sma.LNC_RNC_total[["coef"]][[1]][1,2]
smaplt_total_intercept_upper = model.sma.LNC_RNC_total[["coef"]][[1]][1,3] 
smaplt_total_slope = model.sma.LNC_RNC_total[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_total_slope_lower = model.sma.LNC_RNC_total[["coef"]][[1]][2,2]
smaplt_total_slope_upper = model.sma.LNC_RNC_total[["coef"]][[1]][2,3]

smaplt_tree_intercept = model.sma.LNC_RNC_tree[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_tree_intercept_lower = model.sma.LNC_RNC_tree[["coef"]][[1]][1,2]
smaplt_tree_intercept_upper = model.sma.LNC_RNC_tree[["coef"]][[1]][1,3] 
smaplt_tree_slope = model.sma.LNC_RNC_tree[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_tree_slope_lower = model.sma.LNC_RNC_tree[["coef"]][[1]][2,2]
smaplt_tree_slope_upper = model.sma.LNC_RNC_tree[["coef"]][[1]][2,3]

smaplt_shrub_intercept = model.sma.LNC_RNC_shrub[["coef"]][[1]][1,1] # coef(SMA), elevation
smaplt_shrub_intercept_lower = model.sma.LNC_RNC_shrub[["coef"]][[1]][1,2]
smaplt_shrub_intercept_upper = model.sma.LNC_RNC_shrub[["coef"]][[1]][1,3] 
smaplt_shrub_slope = model.sma.LNC_RNC_shrub[["coef"]][[1]][2,1] # coef(SMA), slope
smaplt_shrub_slope_lower = model.sma.LNC_RNC_shrub[["coef"]][[1]][2,2]
smaplt_shrub_slope_upper = model.sma.LNC_RNC_shrub[["coef"]][[1]][2,3]


smaplt_total_data = traitData_touse %>% dplyr::select(speciesFullName, GrowthForm, SiteID, LNC, RNC) %>%
  dplyr::mutate(RNC = as.numeric(RNC), LNC = as.numeric(LNC))
smaplt_total_data$predicted_LNC = smaplt_total_intercept + smaplt_total_slope * smaplt_total_data$RNC
smaplt_total_data$predicted_LNC_upper_total = smaplt_total_intercept_upper + smaplt_total_slope_upper * smaplt_total_data$RNC
smaplt_total_data$predicted_LNC_lower_total = smaplt_total_intercept_lower + smaplt_total_slope_lower * smaplt_total_data$RNC

smaplt_tree_data = traitData_touse_tree %>% dplyr::select(speciesFullName, GrowthForm, SiteID, LNC, RNC) %>%
  dplyr::mutate(RNC = as.numeric(RNC), LNC = as.numeric(LNC))
smaplt_tree_data$predicted_LNC = smaplt_tree_intercept + smaplt_tree_slope * smaplt_tree_data$RNC
smaplt_tree_data$predicted_LNC_upper_tree = smaplt_tree_intercept_upper + smaplt_tree_slope_upper * smaplt_tree_data$RNC
smaplt_tree_data$predicted_LNC_lower_tree = smaplt_tree_intercept_lower + smaplt_tree_slope_lower * smaplt_tree_data$RNC

smaplt_shrub_data = traitData_touse_shrub %>% dplyr::select(speciesFullName, GrowthForm, SiteID, LNC, RNC) %>%
  dplyr::mutate(RNC = as.numeric(RNC), LNC = as.numeric(LNC))
smaplt_shrub_data$predicted_LNC = smaplt_shrub_intercept + smaplt_shrub_slope * smaplt_shrub_data$RNC
smaplt_shrub_data$predicted_LNC_upper_shrub = smaplt_shrub_intercept_upper + smaplt_shrub_slope_upper * smaplt_shrub_data$RNC
smaplt_shrub_data$predicted_LNC_lower_shrub = smaplt_shrub_intercept_lower + smaplt_shrub_slope_lower * smaplt_shrub_data$RNC
...
model.sma.LNC_RNC_annotlabel = paste0(
  ...
  " P",
  ifelse(
    signif(as.numeric(model.sma.LNC_RNC_total[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.LNC_RNC_total[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.LNC_RNC_total[["pval"]]), digits = 2), "\n"))),
  
  "Tree:R=",
  round(ifelse(
    as.numeric(model.sma.LNC_RNC_tree[["groupsummary"]][["Slope"]]) > 0,
    sqrt(as.numeric(model.sma.LNC_RNC_tree[["r2"]])),
    -sqrt(as.numeric(model.sma.LNC_RNC_tree[["r2"]]))
  ), digits = 2), 
  " P", 
  ifelse(
    signif(as.numeric(model.sma.LNC_RNC_tree[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.LNC_RNC_tree[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.LNC_RNC_tree[["pval"]]), digits = 2), "\n"))),
  
  "Shrub:R=",
  round(ifelse(
    as.numeric(model.sma.LNC_RNC_shrub[["groupsummary"]][["Slope"]]) > 0,
    sqrt(as.numeric(model.sma.LNC_RNC_shrub[["r2"]])),
    -sqrt(as.numeric(model.sma.LNC_RNC_shrub[["r2"]]))
  ), digits = 2),
  " P", 
  ifelse(
    signif(as.numeric(model.sma.LNC_RNC_shrub[["pval"]]), digits = 2)<0.01,
    "<0.01\n", 
    ifelse(signif(as.numeric(model.sma.LNC_RNC_shrub[["pval"]]), digits = 2)<0.05,
           "<0.05\n", 
           paste0("=",signif(as.numeric(model.sma.LNC_RNC_shrub[["pval"]]), digits = 2), "\n")))
)

smaplt.LNC_RNC = ggplot(smaplt_total_data, aes(x=RNC, y=LNC)) +
  geom_point(aes(color=GrowthForm), size=2, alpha=1.0) +  # data points
  stat_function( # total
    fun = function(x) smaplt_total_slope * x + smaplt_total_intercept, 
    xlim = c(min(smaplt_total_data$RNC, na.rm=T), max(smaplt_total_data$RNC, na.rm=T)),
  ...
  labs(subtitle = "RES (Rr25-RNC)", x = "lnRNC", y = "lnRr25") + 
  theme_minimal() + 
  theme(axis.line = element_line(colour = "black")) +
  theme(legend.position = "none") +
  scale_color_manual(values = c("#EECA40", "#7998AD", "#F07673")) +
