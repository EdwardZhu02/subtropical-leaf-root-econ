...
# ------------------------------------------------------------------------------
# Perform PCA analysis (All traits: RES + LES)
...
traitData_bysite_mean = traitDataIndv_SelectedTraits %>%
  ...
  summarise(
    ...
    across(any_of(traitName_touse), ~ mean(.x, na.rm = TRUE)),
    .groups = "drop"
  )
...
nonphyloPCA.screeplot = fviz_screeplot(nonphyloPCAresult, addlabels = TRUE, ylim = c(0, 100),
  ...
  labs(title="PCA, RES+LES (Site-Species Mean)") + theme_classic()
ggsave(filename = "species-level-code/plt_nonphyPCA_scree-bysite.pdf", plot = nonphyloPCA.screeplot, width = 3, height = 3)

# ------------------------------------------------------------------------------
# 2-D PCA biplot: SiteID as grouping factor
# ------------------------------------------------------------------------------

# Define palette for sites
# Hilltop="#547bb4", Valley="#dd7c4f" (alphabetical order: hilltop, valley)
site_palette = c("#547bb4", "#dd7c4f")
...
# Save plot (Axis 1-2 combined)
plt_ax12 = plt_nonphyloPCA_biplot_ax12 + plt_nonphyloPCA_contribplot_ax12 + 
  plot_layout(guides = "collect") & theme(legend.position='top')

ggsave(plot = plt_ax12, filename = "species-level-code/plt_nonphyPCA_ax12Combined_bysite.pdf",
       width = 6.5, height = 4)
...
ggsave(plot = plt_ax23, filename = "species-level-code/plt_nonphyPCA_ax23Combined_bysite.pdf",
       width = 6.5, height = 4)
